...
for LUT_intra_row_idx =187:371
    ...
    filenameParts = string(split(filename, '_'));
    
    if filenameParts(3) == "stair"
        %         cd ..
        cd(string(LUT_intra.expt_date(LUT_intra_row_idx)))
        cd(strjoin(['raw' string(filenameParts(1))],'\'))
...
%         fig_handle = consolidated_plot(time, filtered_data_bp, hes_data, stim_fb, fs);
        ...
        intendedStim = stim.data.load;
        stair_metastruct(iter).P = P;
        stair_metastruct(iter).P.intendedStim = intendedStim(:,1);
        iter = iter+1;
    end
    % Plot data
...
for i = 1:no_of_protocols
    ...
    %
    %             P(i).inc_frq_chirp_f = linspace(0,max_chirp_frq,ON_dur*fs+1);
