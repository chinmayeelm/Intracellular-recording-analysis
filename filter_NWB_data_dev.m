...
function P = plot_data(single_trial_length,no_of_protocols, fs, time, filename,  P, gcfr_max)
    ...
    for i=1:no_of_protocols
        ...
        gcfr_norm = P(i).avg_gcfr/gcfr_max;
        A3 = subplot(4,1,3); plot(time(1:single_trial_length), gcfr_norm, 'Color', [0.2,0.3,0.49]);
        ylabel('Normalised GCFR');
        
        A4 = subplot(4,1,4); plot(time(1:single_trial_length), P(i).stim_hes(1, :), 'Color', [0.6, 0.2,0]);
