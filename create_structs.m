function P = create_structs(rec_protocols_sorted,stim_protocols_hes_sorted,fs, stim_protocols_ifb_sorted, no_of_protocols, no_of_trials, single_trial_length, stim_order_sorted, max_chirp_frq,amp_sweep_frq, blwgn_fc)
    ...
    for i=1:no_of_protocols
        ...
        if (P(i).stim_type == "sin" ) || (P(i).stim_type == "sqr") || (P(i).stim_type == "step")
            ...
            for j=1:no_of_trials
                P(i).antennal_movement(j,:) =  butter_filtfilt(P(i).hes_data_unfilt(j,:), 25, fs, order);
            end
